function [T1, T2] = InitProb8(Nx1, Nx2)
    e1 = ones(1,Nx1);
    e2 = ones(1,Nx2);
    ...
    left = zeros(2,Nx2*2-1); right = zeros(2,Nx2*2-1); 
    top = [linspace(0,0.5,Nx1); e1];
    left(:,1:Nx2) = [linspace(1,0,Nx2); 0*e2];
    left(:,Nx2:end) = [0*e2; linspace(0,1,Nx2)];
    right(:,1:Nx2) = [linspace(1,0.5,Nx2); 0.5*e2];
    right(:,Nx2:end) = [0.5*e2 ; linspace(0.5,1,Nx2)];
    bottom = [e1; linspace(0,0.5,Nx1)];
    top = flip(top,2);
    bottom = flip(bottom,2);
